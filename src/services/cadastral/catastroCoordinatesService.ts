...
export const getCadastralInfoFromCoordinates = async (
  ...
  try {
    ...
    const cachedData = await getCachedCadastralData(latitude, longitude);
    if (cachedData) {
...
export const refreshCadastralData = async (coordinates: GeoCoordinates): Promise<CatastroData> => {
  try {
    ...
    console.log(
      `Rafraîchissement des données cadastrales pour: ${coordinates.lat}, ${coordinates.lng}`,
    );
    ...
    // Récupérer les données cadastrales via l'API REST
    const cadastralData = await getCadastralDataByCoordinatesREST(coordinates.lat, coordinates.lng);
    ...
    const utmCoordinates = coordinates
      ? getFormattedUTMCoordinates(coordinates.lat, coordinates.lng)
      : '';
