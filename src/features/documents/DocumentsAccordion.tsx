...
const DocumentsAccordion = ({ documents, onDocumentAction, title }: DocumentsAccordionProps) => {
  ...
  const statusCounts = {
    ...
    generated: allDocuments.filter((doc) => doc.status === 'generated' || doc.status === 'linked')
      .length,
    ready: allDocuments.filter((doc) => doc.status === 'ready').length,
    pending: allDocuments.filter(
      (doc) => doc.status === 'pending' || doc.status === 'action-required',
    ).length,
        ...
        {allDocuments.map((document) => (
          ...
          >
                    ...
                    <div className="flex items-center gap-1">
                      ...
                      <DropdownMenu>
                        ...
                        <DropdownMenuContent align="end">
                          <DropdownMenuItem
                            onClick={() => onDocumentAction(document.id, 'regenerate')}
                          >
                            Régénérer le document
                          </DropdownMenuItem>
                          {document.type === 'ficha' && (
                            <DropdownMenuItem
                              onClick={() => onDocumentAction(document.id, 'refresh-ocr')}
                            >
                          ...
                          {document.type === 'certificado' && (
                            <DropdownMenuItem
                              onClick={() => onDocumentAction(document.id, 'update-cee')}
                            >
            ...
            <AccordionContent className="px-0">
              <DocumentAccordionContent document={document} onAction={onDocumentAction} />
            </AccordionContent>
          </AccordionItem>
        ))}
      </Accordion>
...
const DocumentAccordionContent = ({
          ...
          <div className="space-y-4">
            <div className="bg-blue-50 p-4 rounded-md">
              ...
              <p className="text-sm text-blue-600">
                Toutes les données sont disponibles pour générer ce document.
              </p>
            </div>
            ...
            {document.statusLabel?.includes('CEE') && (
              <Button
                onClick={() =>
                  onAction(
                    document.id,
                    document.type === 'certificado' ? 'update-cee' : 'refresh-ocr',
                  )
                }
              ...
              >
                ⚙️{' '}
                {document.type === 'ficha' ? 'Traiter CEE PREVIO (OCR)' : 'Actualiser depuis CEE'}
              </Button>
            )}
      ...
      case 'action-required':
        return (
          <div className="bg-amber-50 p-4 rounded-md">
            ...
            <p className="text-sm text-amber-600">
              {document.statusLabel || 'Une action est requise'}
            </p>
            {document.type === 'fotos' && (
              <Button
  ...
  return <div className="py-4 px-4 border-t bg-gray-50">{renderContextualContent()}</div>;
};
