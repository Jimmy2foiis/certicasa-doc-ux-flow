...
import TemplateVariableMapping from '@/features/documents/template-mapping/TemplateVariableMapping';
...
const ClientDocumentGenerator = ({
  ...
  const validateTemplate = useCallback((template: any) => {
    ...
    if (template.type === 'pdf') {
      return (
        template.content.startsWith('data:application/pdf') || template.content.startsWith('blob:')
      );
    }
  ...
  const handleTemplateSelect = (templateId: string) => {
    ...
    if (!isValid) {
      toast({
        title: 'Avertissement',
        description:
          'Le modèle sélectionné est vide ou contient un contenu invalide. La génération de document pourrait échouer.',
        variant: 'default',
      });
    }
  ...
  const validateMapping = (): { valid: boolean; message: string } => {
    ...
    const unmappedTags = templateMappings.filter(
      (tag) => !tag.mappedTo || tag.mappedTo.trim() === '',
    );
  ...
  const handleDocumentGeneration = async () => {
    ...
    if (!templateValid) {
      toast({
        title: 'Erreur',
        description:
          'Le modèle sélectionné est invalide ou vide. Impossible de générer un document.',
        variant: 'destructive',
      });
      return;
    }
          ...
          {generated ? (
            ...
            <div className="py-8 text-center">
              <div className="animate-pulse flex flex-col items-center">
                ...
                <p className="mt-4 font-medium">Génération du document en cours...</p>
              </div>
