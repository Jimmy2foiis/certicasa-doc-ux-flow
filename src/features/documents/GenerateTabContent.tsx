        ...
        <div className="text-center p-8">
          ...
          {selectedTemplate ? (
            <div className="mb-8">
              ...
              <div className="p-4 border rounded-md bg-gray-50 inline-block">
                <div className="flex items-center gap-3">
                  <div
                    className={`p-2 rounded-full ${selectedTemplate.type === 'docx' ? 'bg-blue-100' : 'bg-red-100'}`}
                  >
                    <FileText
                      className={`h-5 w-5 ${selectedTemplate.type === 'docx' ? 'text-blue-600' : 'text-red-600'}`}
                    />
                  </div>
                  <div className="text-left">
                    <p className="font-medium">{selectedTemplate.name}</p>
                    <p className="text-xs text-gray-500">
                      {selectedTemplate.type.toUpperCase()} - Ajouté le{' '}
                      {selectedTemplate.dateUploaded}
                    </p>
                  </div>
                </div>
              </div>
            ...
            <p className="text-gray-500 mb-8 max-w-md mx-auto">
              Utilisez cette section pour générer de nouveaux documents à partir des modèles
              disponibles. Vous pouvez également ajouter de nouveaux modèles pour les documents
              spécifiques.
            </p>
          )}

          <div className="flex flex-col sm:flex-row gap-4 justify-center">
            <GenerateDocumentButton fullWidth templateId={selectedTemplate?.id} />
