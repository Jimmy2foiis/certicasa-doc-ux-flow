    ...
    if (isOpen && document) {
      ...
      try {
        ...
        if (fileType !== 'pdf') {
          ...
          if (fileType === 'docx') {
            setError(
              `La prévisualisation n'est pas disponible pour les fichiers DOCX. Veuillez télécharger le document.`,
            );
          } else {
            setError(
              `La prévisualisation n'est pas disponible pour les fichiers ${fileType.toUpperCase()}. Veuillez télécharger le document.`,
            );
          }
          return;
        }
        ...
        if (document.content) {
          const previewResult = documentService.createDocumentPreviewUrl(
            document.content,
            document.type,
          );
        ...
        setError(
          `Impossible de charger le document: ${err instanceof Error ? err.message : String(err)}`,
        );
        toast({
          title: 'Erreur',
