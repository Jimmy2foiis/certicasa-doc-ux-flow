...
export const loadTemplateMapping = async (templateId: string): Promise<TemplateTag[]> => {
  try {
    ...
    if (mappingData?.mappings) {
      ...
      if (
        Array.isArray(jsonMappings) &&
        jsonMappings.length > 0 &&
        typeof jsonMappings[0] === 'object' &&
        jsonMappings[0] !== null &&
        'tag' in jsonMappings[0] &&
        'category' in jsonMappings[0] &&
        'mappedTo' in jsonMappings[0]
      ) {
...
export const saveTemplateMapping = async (
  ...
  try {
    ...
    const { error } = await supabase.from('template_mappings').upsert({
...
export const createInitialMapping = (content: string | null): TemplateTag[] => {
