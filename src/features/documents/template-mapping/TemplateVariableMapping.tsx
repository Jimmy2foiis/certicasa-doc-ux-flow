  ...
  useEffect(() => {
    ...
    try {
      ...
      if (extractedTags.length === 0) {
        setError(
          "Aucune variable n'a été détectée dans ce modèle. Vous pouvez ajouter des variables manuellement.",
        );
      }
      ...
      setError(
        error instanceof Error ? error.message : "Erreur lors de l'extraction des variables",
      );
      setLoading(false);
    ...
    try {
      ...
      while ((matches = tagRegex.exec(workingContent)) !== null) {
        ...
        if (!tags.some((t) => t.tag === tag)) {
          ...
          if (
            tagInside.toLowerCase().includes('projet') ||
            tagInside.toLowerCase().includes('project')
          ) {
            category = 'project';
        ...
        {loading ? (
          ...
          <>
            ...
            <div className="mt-6">
              ...
              <TagsList {...mappingContentProps} tags={templateTags} />
            </div>
            ...
            <div className="mt-6 flex justify-between items-center">
              <div className="text-sm text-muted-foreground">
                {templateTags.length === 0 ? (
                  ...
                  <span>
                    {templateTags.filter((t) => t.mappedTo).length}/{templateTags.length} variables
                    associées
                  </span>
                )}
              </div>
