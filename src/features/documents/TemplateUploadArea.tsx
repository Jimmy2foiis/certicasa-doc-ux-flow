...
const TemplateUploadArea = ({
  ...
  const handleDrop = (e: React.DragEvent<HTMLDivElement>) => {
    ...
    if (e.dataTransfer.files && e.dataTransfer.files.length > 0) {
      ...
      if (invalidFiles.length > 0) {
        ...
        setError(`Format(s) non accepté(s): ${invalidNames}. Formats supportés: ${supportedFormats.join(', ')}`);
        return;
      }
  ...
  return <div className="space-y-3">
      ...
      <div className={`border-2 border-dashed rounded-lg p-6 text-center transition-colors duration-200 ${uploading ? 'border-gray-300 bg-gray-50 opacity-75 cursor-not-allowed' : dragActive ? 'border-primary bg-primary/5 cursor-pointer' : 'border-gray-300 bg-gray-50 cursor-pointer hover:border-primary hover:bg-primary/5'}`} onDragOver={handleDragOver} onDragLeave={handleDragLeave} onDrop={handleDrop} onClick={uploading ? undefined : handleButtonClick}>
        <input ref={fileInputRef} type="file" id="file-upload" className="hidden" accept={acceptString} multiple onChange={handleChange} disabled={uploading} />
        <div className="flex flex-col items-center justify-center">
          {uploading ? <div className="animate-pulse">
              ...
              <div className="flex flex-wrap justify-center gap-2 mb-4">
                {supportedFormats.map(format => <Badge key={format} variant="outline" className="flex items-center gap-1">
                    <FileText className="h-3 w-3" />
                    {format}
                  </Badge>)}
              </div>
              <Button type="button" disabled={uploading} className="bg-green-600 hover:bg-green-700">
