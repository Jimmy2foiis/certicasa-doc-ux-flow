...
import type { DocumentTemplate } from '@/types/documents'; // Import from central type definition
    ...
    <Dialog open={showPreview} onOpenChange={closePreview}>
      <DialogContent className="max-w-4xl">
        <DialogHeader>
          ...
          <DialogDescription>Ajouté le {previewTemplate?.dateUploaded}</DialogDescription>
        </DialogHeader>
        ...
        <div className="relative min-h-[300px] border rounded-md flex items-center justify-center">
          <div className="text-center">
            ...
            <p className="text-sm text-gray-400 mt-2">
              L'aperçu des fichiers {previewTemplate?.type} n'est pas disponible directement dans
              l'application.
            </p>
          </div>
        </div>
        ...
        <DialogFooter className="sm:justify-between flex flex-col sm:flex-row gap-2">
          <div>
            <p className="text-sm text-gray-500">Type: {previewTemplate?.type.toUpperCase()}</p>
          </div>
