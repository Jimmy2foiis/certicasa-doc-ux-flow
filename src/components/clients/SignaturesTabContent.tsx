...
const SignaturesTabContent = ({
  ...
  const toggleSignatureSelection = (signatureId: string) => {
    setSelectedSignatures(prev => prev.includes(signatureId) ? prev.filter(id => id !== signatureId) : [...prev, signatureId]);
  };
  ...
  return <Card>
      <CardHeader className="flex flex-row items-center justify-between">
        ...
        <div className="flex gap-2">
          {selectedSignatures.length > 0 && <Button variant="outline" size="sm">
      ...
      <CardContent>
        <div className="space-y-4">
          {signatures.length === 0 ? <div className="text-center py-6 text-gray-500">
              Aucune signature trouvée pour ce client
            </div> : signatures.map(signature => <div key={signature.id} className="flex items-center justify-between p-3 border rounded-md hover:bg-gray-50">
                <div className="flex items-center gap-3">
                  <Checkbox checked={selectedSignatures.includes(signature.id)} onCheckedChange={() => toggleSignatureSelection(signature.id)} id={`signature-${signature.id}`} />
                ...
                <div className="flex items-center gap-2">
                  ...
                  {signature.status === "signed" && <Button variant="outline" size="sm">
      ...
      <CardFooter className="flex justify-between border-t pt-5">
        ...
        {signatures.some(s => s.status === "pending") && <Button variant="outline" size="sm">
            Envoyer un rappel
          </Button>}
      </CardFooter>
    </Card>;
